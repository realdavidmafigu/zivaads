...
-- This script safely handles the "column specified more than once" error
...
-- Step 2: Check if campaign_metrics table exists and what it contains
DO $$
DECLARE
...
BEGIN
    ...
    IF table_exists THEN
        ...
        RAISE NOTICE 'campaign_metrics table exists with % columns', column_count;
        
        -- Show campaign_metrics structure
        RAISE NOTICE 'campaign_metrics columns:';
        FOR col IN 
        ...
        END LOOP;
    ELSE
        RAISE NOTICE 'campaign_metrics table does not exist';
    END IF;
END $$;

-- Step 3: Drop campaign_metrics table if it exists (to avoid conflicts)
DROP TABLE IF EXISTS campaign_metrics CASCADE;

-- Step 4: Create campaign_metrics table cleanly
...
-- Step 5: Create indexes for campaign_metrics
...
-- Step 6: Migrate existing performance data from campaigns table
DO $$
DECLARE
...
-- Step 7: Create function to mark latest metrics
...
-- Step 8: Create trigger for latest metrics
...
-- Step 9: Enable RLS and create policies for campaign_metrics
...
-- Step 13: Test the backward compatibility view
SELECT 
...
-- Step 14: Show final table structures
SELECT 
