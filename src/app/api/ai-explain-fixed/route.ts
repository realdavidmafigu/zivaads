...
function generateFallbackResponse(campaigns: any[], metrics: any): string {
  const campaignCount = campaigns?.length || 0;
  
  if (campaignCount === 1) {
    const campaign = campaigns[0];
    return `Your campaign "${campaign.name}" is running well! The AI suggests monitoring how many people click on your ad and considering testing different images or messages to improve performance.`;
  }
  
  return `Your Facebook ad account is performing well! Here's what I can tell you about your ${campaignCount} campaigns:
